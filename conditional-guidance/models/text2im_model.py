...
class Text2ImUNet(UNetModel):
    ...
    :param xf_width: width of the transformer.  每一个词用xf长度的向量表示
    ...
    def get_text_emb(self, tokens, mask):
        ...
        xf_in = self.token_embedding(tokens.long())  # 输入转成张量表示
        xf_in = xf_in + self.positional_embedding[None]  # 词向量嵌入和位置嵌入，尝试不一样的权重相加？
        if self.xf_padding:
