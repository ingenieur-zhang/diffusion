...
# from Diffusion.pecgn import PEGCN_Unet
# from Trans_scrpit import Transformer_gnn
# from Diffusion import Diffusion_ddim_gnn
...
class IntpDataset(Dataset):
    def __init__(self, settings, mask_distance, call_name):
        ...
        if call_name == 'train':
            ...
            # print('call_scene_list:  ', len(call_scene_list))
    ...
    def __getitem__(self, idx):
        ...
        if self.call_name in ['train', 'train_self']:
            ...
            graph_candidates_1 = df[df['op'] == f's_label_{self.call_list[idx][1]}'][
        ...
        for op in aggregated_df['op'].unique():
        ...
        # print(coords)
        # print(answers)
        return features, coords, answers, self.tif
...
def collate_fn(examples):
    ...
    x_pm = x_b[:, :, 0:1]
    q_tr = torch.cat((c_b, x_b), dim=2)
    # q_gnn = torch.cat((c_b, x_pm), dim=2)
    tif = pad_sequence([ex[3] for ex in examples if len(ex[3]) > 2], batch_first=True, padding_value=0.0)
    return x_b, c_b, y_b, x_pm, q_tr, input_lenths, tif
...
def collate_fn_img(examples):
    ...
    # q_gnn = torch.cat((c_b, x_pm), dim=2)
    tif = pad_sequence([ex[3] for ex in examples if len(ex[3]) > 2], batch_first=True, padding_value=0.0)
    target_tensor[torch.arange(batch_size).unsqueeze(1),  coord[:,:,0],  coord[:,:,1]]  =  value.squeeze(-1)
    return x_b, c_b, y_b, x_pm, q_tr, input_lenths, tif
