...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1):
        ...
        self.d_k = d_model // h
        self.h = h
        self.linears = clones(nn.Linear(d_model, d_model), 4)  # 4 stacked FC layers with d_model
        self.attn = None
    ...
    def forward(self, query, key, value, mask=None):
        ...
        # QKV go through a FC layer, output a 4d matrix(B,h,F: words in a sentence,d_k:embedding length),
        query, key, value = [l(x).view(nbatches, -1, self.h, self.d_k).transpose(1, 2) for l, x in zip(self.linears, (query, key, value))]
        # 2) Apply attention on all the projected vectors in batch.
        # x: weight matrix, attn: P matrix
